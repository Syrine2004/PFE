...
@CrossOrigin(origins = "*") // À configurer via API Gateway idéalement
public class ConcoursController {
    ...
    public ResponseEntity<PageResponseDTO<ConcoursResponseDTO>> getConcours(
            ...
            @RequestParam(required = false) StatutResultat statutResultat,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(concoursService.getConcours(annee, typeConcours, statutResultat, page, size));
    }
