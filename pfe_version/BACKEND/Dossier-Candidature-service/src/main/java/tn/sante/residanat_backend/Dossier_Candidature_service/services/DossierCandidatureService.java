...
public class DossierCandidatureService {
    ...
    public void handleIAResponse(java.util.Map<String, Object> iaResponse) {
        ...
        if (dossier != null) {
            ...
            // On filtre les anciennes anomalies : 
            // 1. On retire celles du même type (préfixe) pour les remplacer par les nouvelles
            // 2. On retire les messages "Aucune anomalie" génériques
            ...
            if (!currentAnomalies.isBlank()) {
                for (String line : currentAnomalies.split("\n")) {
                    ...
                    if (!cleanLine.startsWith(prefix) && 
                        !cleanLine.toLowerCase().contains("aucune anomalie")) {
                        anomalyLines.add(cleanLine);
                    }
                }
            }
            
            // Ajout des nouvelles anomalies si présentes ET (score < 100% ou erreur de score)
            if (newScore == null || newScore < 100.0) {
                boolean hasSpecificAnomalies = newAnomaliesStr != null && !newAnomaliesStr.isBlank() && 
                    !newAnomaliesStr.toLowerCase().contains("aucune anomalie");
                
                if (hasSpecificAnomalies) {
                    ...
                    anomalyLines.add(prefix + "Données peu lisibles ou incomplètes (Vérification manuelle requise).");
                }
            }
            
            // Finalisation de la chaîne d'anomalies
    ...
    public List<DossierCandidature> getAllDossiers() {
        return dossierRepository.findAllByStatutNot(StatutDossier.BROUILLON);
    }
    ...
    public DossierCandidature updateStatut(Long id, StatutDossier statut) {
        ...
        // Éviter l'envoi de multiples notifications si le statut ne change pas
        ...
        if (statut == StatutDossier.VALIDE) {
            ...
            DossierRejeteEvent event = new DossierRejeteEvent(
                savedDossier.getId(),
                savedDossier.getCandidatId(),
                savedDossier.getConcoursId() != null ? savedDossier.getConcoursId().toString() : null,
    ...
    @org.springframework.amqp.rabbit.annotation.RabbitListener(queues = RabbitMQConfig.QUEUE_CONVOCATION_READY)
    @Transactional
    ...
    public long countByStatut(StatutDossier statut) {
        return dossierRepository.countByStatut(statut);
    }

    public long countByStatut(StatutDossier statut, UUID concoursId) {
        return dossierRepository.countByStatutAndConcoursId(statut, concoursId);
    }

    public long countTotal() {
        return dossierRepository.countByStatutNot(StatutDossier.BROUILLON);
    }

    public long countTotal(UUID concoursId) {
        return dossierRepository.countByStatutNotAndConcoursId(StatutDossier.BROUILLON, concoursId);
    }
    ...
    public long countIAEvaluations() {
        return evaluationRepository.count();
    }

    public java.util.List<Long> getCandidatIdsByConcours(UUID concoursId) {
        if (concoursId == null) {
            return dossierRepository.findAllCandidatIds();
        }
        return dossierRepository.findCandidatIdsByConcoursId(concoursId);
    }

    public java.util.List<java.util.Map<String, Object>> getValidatedDossiers(java.util.UUID concoursId) {
        return dossierRepository.findByConcoursIdAndStatut(concoursId, StatutDossier.VALIDE)
            .stream()
            .map(d -> {
                java.util.Map<String, Object> map = new java.util.HashMap<>();
                map.put("dossierId", d.getId());
                map.put("candidatId", d.getCandidatId());
                return map;
            })
            .collect(java.util.stream.Collectors.toList());
    }

    @RabbitListener(queues = RabbitMQConfig.QUEUE_CONCOURS_DELETED)
    @Transactional
    public void handleConcoursDeleted(java.util.Map<String, Object> event) {
        String concoursIdStr = (String) event.get("concoursId");
        log.info("📢 Réception événement concours.deleted pour ID: {}", concoursIdStr);
        if (concoursIdStr != null) {
            try {
                UUID concoursId = UUID.fromString(concoursIdStr);
                dossierRepository.deleteAllByConcoursId(concoursId);
                log.info("✅ Tous les dossiers pour le concours {} ont été supprimés.", concoursId);
            } catch (Exception e) {
                log.error("❌ Erreur lors du nettoyage des dossiers pour le concours {}: {}", concoursIdStr, e.getMessage());
            }
        }
    }
