...
public interface ConvocationRepository extends JpaRepository<Convocation, Long> {
     ...
     */
    @Query("SELECT COUNT(c) FROM Convocation c WHERE (:concoursId IS NULL OR c.concoursId = :concoursId) AND c.statut = 'GENEREE'")
    long countGeneratedConvocations(@Param("concoursId") String concoursId);
    ...
    /**
     * Supprimer toutes les convocations d'un concours
     * 
     * @param concoursId l'identifiant du concours
     */
