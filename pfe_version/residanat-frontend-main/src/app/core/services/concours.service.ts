...
export class ConcoursService {
    ...
    private selectedConcoursIdSubject = new BehaviorSubject<string | null>(localStorage.getItem('selectedConcoursId'));
    public selectedConcoursId$ = this.selectedConcoursIdSubject.asObservable();

    public setSelectedConcoursId(id: string | null) {
        this.selectedConcoursIdSubject.next(id);
        if (id) {
            localStorage.setItem('selectedConcoursId', id);
        } else {
            localStorage.removeItem('selectedConcoursId');
        }
    }

    public getSelectedConcoursId(): string | null {
        return this.selectedConcoursIdSubject.value;
    }
    ...
    createConcours(concours: any): Observable<Concours> {
        ...
        const payload = {
            ...
            etat: (concours.statut === 'publie' || concours.actif) ? 'PUBLIE' : 'NON_PUBLIE',
