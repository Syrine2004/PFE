...
export class Centre3dComponent implements OnInit, OnDestroy {
  ...
  private centers: CenterDef[] = [];

  private initLabels(): void {
    this.centers = [
      {
        key: 'tunis', label: 'Tunis', facultyName: this.translate.instant('CENTRE_3D.FACULTES.TUNIS'),
        lat: 36.8065, lng: 10.1815, levels: 6, entrance: '15 Rue Djebel Lakhdhar', occupancy: 67
      },
      {
        key: 'sfax', label: 'Sfax', facultyName: this.translate.instant('CENTRE_3D.FACULTES.SFAX'),
        lat: 34.7406, lng: 10.7603, levels: 5, entrance: 'Avenue Majida Boulila', occupancy: 55
      },
      {
        key: 'sousse', label: 'Sousse', facultyName: this.translate.instant('CENTRE_3D.FACULTES.SOUSSE'),
        lat: 35.8256, lng: 10.6369, levels: 5, entrance: 'Avenue Mohamed Karoui', occupancy: 85
      },
      {
        key: 'monastir', label: 'Monastir', facultyName: this.translate.instant('CENTRE_3D.FACULTES.MONASTIR'),
        lat: 35.7643, lng: 10.8113, levels: 4, entrance: 'Avenue Fattouma Bourguiba', occupancy: 20
      }
    ];
    if (this.map) this.drawAllCenters();
  }
  ...
  ngOnInit(): void { 
    ...
    // Single robust stream for concours and assignment
    this.concoursSub = this.concoursService.selectedConcoursId$.pipe(
      switchMap(globalCId => {
        // If globalCId is explicitly set to '' (All Concours), we return it
        if (globalCId !== null) return of(globalCId);
        
        // Otherwise (initial load), we fetch the first published one as fallback
        return this.concoursService.getConcours(0, 1, undefined, undefined, 'PUBLIE').pipe(
          map(res => res.content?.[0]?.id || '')
        );
      }),
      switchMap(cid => {
        if (!cid) return of(null); // Explicit reset if cid is empty
        return this.authService.getProfile().pipe(
          switchMap(profile => {
             if (!profile?.id) return of(null);
             return this.dossierService.getDossierByCandidat(profile.id, cid).pipe(
               switchMap(dossier => {
                 if (!dossier?.id) return of(null);
                 return this.dossierService.getConvocationInfo(dossier.id).pipe(
                   map(resp => resp.body) // Map to the body directly
                 );
               }),
               catchError(() => of(null))
             );
      ...
      this.ngZone.run(() => {
        if (convocation && convocation.lieuExamenDetail) {
          const normalizedLieu = this.normalizeText(convocation.lieuExamenDetail);
          let targetKey: string | null = null;
          
          if (normalizedLieu.includes('sfax')) targetKey = 'sfax';
          else if (normalizedLieu.includes('sousse')) targetKey = 'sousse';
          else if (normalizedLieu.includes('monastir')) targetKey = 'monastir';
          else if (normalizedLieu.includes('tunis')) targetKey = 'tunis';
  ...
  getOccupancyState(occupancy: number): string {
    if (occupancy <= 30) return this.translate.instant('CENTRE_3D.OCCUPANCY_STATE.LOW');
    if (occupancy <= 70) return this.translate.instant('CENTRE_3D.OCCUPANCY_STATE.MEDIUM');
    return this.translate.instant('CENTRE_3D.OCCUPANCY_STATE.HIGH');
  }
  ...
  private async initCesium(): Promise<void> {
    ...
    if (!Cesium || !container) {
      this.cesiumError = this.translate.instant('CENTRE_3D.CESIUM_ERROR');
    ...
    this.watchId = navigator.geolocation.watchPosition(
      pos => {
        if (!this.manualOrigin) {
          ...
          this.updateUserMarker();
          if(!this.routeLine) this.tryBuildRoute(); 
        }
      },
      (err) => {
  ...
  async tryBuildRoute(): Promise<void> {
    if (!this.userLocation || !this.map || !this.destination) return;
    
    const bProfile = this.travelMode === 'walking' ? 'foot-light' : 'car-fast';
    const lonlats = `${this.userLocation.lng},${this.userLocation.lat}|${this.destination.lng},${this.destination.lat}`;

    try {
      const response = await fetch(`/osrm-local/brouter?lonlats=${lonlats}&profile=${bProfile}&alternativeidx=0&format=geojson`);
      ...
      if (response.ok) {
        ...
        if (data?.features?.length) {
          ...
          latLngs = routeFeature.geometry.coordinates.map((c: [number, number]) => [c[1], c[0]]);
      ...
      if (latLngs.length === 0) {
        // --- FALLBACK MOCK LOGIC ---
        console.warn('Routing server unreachable. Using Mock Route.');
        const steps = 15;
        for (let i = 0; i <= steps; i++) {
          const ratio = i / steps;
          latLngs.push([
            this.userLocation.lat + (this.destination.lat - this.userLocation.lat) * ratio,
            this.userLocation.lng + (this.destination.lng - this.userLocation.lng) * ratio
          ]);
        }
        const dx = (this.destination.lng - this.userLocation.lng) * 111;
        const dy = (this.destination.lat - this.userLocation.lat) * 111;
        distance = Math.sqrt(dx*dx + dy*dy) * 1000;
        duration = (distance / 13) * 60;
      }

      this.renderRoute(latLngs, distance, duration);

    } catch (err) {
      console.error('Network Error, activating Mock Mode:', err);
      ...
      const duration = (distance / 13) * 60;
      this.renderRoute(latLngs, distance, duration);
    }
  }

  private renderRoute(latLngs: [number, number][], distance: number, duration: number): void {
    ...
    this.routeDepthLine = L.polyline(latLngs, {
      color: '#334155', opacity: 0.35, weight: 9, lineCap: 'round'
    }).addTo(this.map);
      ...
      <div class="vehicle-icon-wrapper">
        <div class="vehicle-body-3d ${this.travelMode}" style="position:relative;width:65px;height:85px;display:flex;align-items:center;justify-content:center;transform-origin:center center;transition:transform 0.3s ease;will-change:transform;">
          ${this.travelMode === 'driving'
            ? '<img class="driving-img" src="assets/images/car_model.png" style="width:60px;height:60px;object-fit:contain;"/>'
            : '<img class="walking-img" src="assets/images/doctor_walk.gif" style="width:65px;height:85px;object-fit:contain;display:block;"/>'
          }
        </div>
      </div>
    `;
    const vehicleIcon = L.divIcon({
      className: 'custom-vehicle-icon', html: vehicleHtml, iconSize: [60, 80], iconAnchor: [30, 65]
    });
    ...
    this.routeDistanceKm = Number((distance / 1000).toFixed(2));
    let finalMinutes = Math.round(duration / 60);

    if (this.travelMode === 'driving') finalMinutes = Math.round(finalMinutes * 1.8);
    else {
      const minWalkingMinutes = Math.round(this.routeDistanceKm * 13);
      if (finalMinutes < minWalkingMinutes) finalMinutes = minWalkingMinutes;
    }
    ...
    this.arrivalTime = arrivalDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', hour12: false });
