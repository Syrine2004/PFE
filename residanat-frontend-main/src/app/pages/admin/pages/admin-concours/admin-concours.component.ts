...
export class AdminConcoursComponent implements OnInit {
    ...
    private mapToFrontend(c: Concours): Concours {
        return {
            ...
            titre: c.libelle,
            type: c.typeConcours,
            statut: c.statutResultat ? c.statutResultat.toLowerCase() : 'non_publie'
        };
    }

    loadConcours() {
        this.concoursService.getConcours(this.currentPage, this.pageSize).subscribe({
            next: (response) => {
                this.concoursList = response.content.map(c => this.mapToFrontend(c));
            ...
            error: (err) => {
                ...
                this.showAlert({
                    ...
                    message: 'Impossible de récupérer la liste des concours.',
                    confirmText: 'Fermer'
                });
            }
        });
    ...
    get stats() {
        return {
            ...
            // Not handled by backend specifically but keep UI
            actifs: this.concoursList.filter(c => !c.deleted).length
        };
    }
    ...
    deleteConcours(id: string | undefined) {
        ...
        this.showAlert({
            ...
            message: 'Êtes-vous sûr de vouloir supprimer ce concours ? Cette action est un "soft delete".',
    ...
    toggleActif(concours: Concours) {
        // Since actif is not yet mapped to backend, we just toggle locally for UI
        concours.actif = !concours.actif;
        this.closeDropdown();
    }
}
